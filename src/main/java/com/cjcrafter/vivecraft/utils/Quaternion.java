package com.cjcrafter.vivecraft.utils;

import org.bukkit.util.Vector;
import com.cjcrafter.vivecraft.utils.lwjgl.Matrix3f;
import com.cjcrafter.vivecraft.utils.lwjgl.Matrix4f;
...
public class Quaternion {
    ...
    public Quaternion(float pitch, float yaw, float roll, Angle.Order order) {
        ...
        switch (order) {
            case XYZ:
                quat = quatX.multiply(quatY).multiply(quatZ);
                break;
            case ZYX:
                quat = quatZ.multiply(quatY).multiply(quatX);
                break;
            case YXZ:
                quat = quatY.multiply(quatX).multiply(quatZ);
                break;
            case ZXY:
                quat = quatZ.multiply(quatX).multiply(quatY);
                break;
            case YZX:
                quat = quatY.multiply(quatZ).multiply(quatX);
                break;
            case XZY:
                quat = quatX.multiply(quatZ).multiply(quatY);
                break;
        }
        w = quat.w;
        x = quat.x;
